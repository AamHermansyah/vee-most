...
const CardProduct = ({ data }) => {
  return (
    <Box
      className="col-span-1 relative min-h-[350px] rounded-xl bg-white overflow-hidden cursor-pointer active:scale-95 transition-transform duration-200"
      sx={{
        boxShadow:
    ...
    >
      {data?.image && (
        <div className="w-full h-[200px] overflow-hidden p-3">
          <Image
            src={data?.image}
            alt={data.name}
      ...
      <div className="min-h-[75px] flex flex-col justify-between px-2 sm:px-10 py-2 sm:py-8">
        <Typography
          variant="Name"
          className="cursor-pointer text-center text-[24px] font-bold font-primary transition-colors duration-200"
        >
          {data.name}
        </Typography>
        <Typography
